...
alter table board add foreign key (help_name) references users(username);

--ALTER TABLE [FK를 생성시킬 테이블명]
--ADD CONSTRAINT [FK명] foreign KEY(FK가 될 컬럼명) references [PK가 위치하는 테이블] ([PK컬럼명]);

insert into board values (board_no_seq.nextval, 'user', '제목테스트', '1234', '제목내용', 0, sysdate);

select * from board order by help_no desc;

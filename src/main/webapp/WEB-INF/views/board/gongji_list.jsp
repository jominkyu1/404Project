...
${b.board_title}
		...
		<td align="center"><input type="button" value="수정"
			onclick="location=
'gongji_cont?no=${b.board_no}&page=${page}&state=edit';" />
								<input type="button" value="삭제"
								onclick="if(confirm('정말로 삭제할까요?') == true){
location=
'gongji_del?no=${b.board_no}&page=${page}';	
}else{ return ;}" />
...
취소를 클릭하면 false를 반환 --%></td>
    ...
    [다음]
     ...
     [이전]&nbsp;
					...
					<c:if test="${page>1}">
						<a
							href="gongji_list?page=${page-1}&find_field=${find_field}&find_name=${find_name}">[이전]</a>&nbsp;
    </c:if>
    ...
    [다음]
					...
					<c:if test="${page<maxpage}">
						<a
							href="gongji_list?page=${page+1}&find_field=${find_field}&find_name=${find_name}">[다음]</a>
					</c:if>
				</c:if>
			</div>
			...
			<div id="bList_menu">
				<input type="button" value="글쓰기"
					onclick="location='gongji_write?page=${page}';" />
				<c:if test="${(!empty find_field) && (!empty find_name)}">
				</c:if>
			</div>
